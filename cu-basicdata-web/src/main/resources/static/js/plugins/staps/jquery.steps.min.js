...
! function(a, b) {
	...
	function A(b, c, d) {
		if(d.stepCount > 0) {
			...
			if(!c.enableContentCache || !e.contentLoaded) switch(r($, e.contentMode)) {
				case $.iframe:
					b.find(".content > .body").eq(d.currentIndex).empty().html('<iframe src="' + e.contentUrl + '" frameborder="0" scrolling="no" />').data("loaded", "1");
					break;
				case $.async:
	...
	function L(a, b, c) {
		if(b.enablePagination) {
			var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}<li style="display:none" class="prequalification"><a  role="menuitem">预审</a></li></ul></{0}>',
