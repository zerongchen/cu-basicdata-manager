...
LPB.plugins['text'] = function (active_component,leipiplugins) {
  ...
  //右弹form  取消控件
  $(popover).delegate(".btn-white", "click", function(e){
      ...
      $.each(inputs, function(i,e){
          ...
          switch(attr_name)
          {
            ...
            case 'orgname':
              ...
              active_component.find(".control-label").text(attr_val+"：");
              break;
            default:
...
LPB.plugins['password'] = function (active_component,leipiplugins) {
	  var plugins = 'text',popover = $(".popover");
	  //右弹form  初始化值
	  $(popover).find("#orgname").val($(leipiplugins).attr("title"));
	  $(popover).find("#orgvalue").val($(leipiplugins).val());
