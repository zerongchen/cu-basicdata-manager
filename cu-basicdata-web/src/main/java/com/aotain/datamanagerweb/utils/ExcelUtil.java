...
import com.aotain.cu.serviceapi.model.DataErrorInfo;
import com.aotain.datamanagerweb.utils.dataimport.*;
...
public class ExcelUtil {
	...
	public static Map<Integer, Map<Integer, String[]>> readExcelFromStream(InputStream input) {
		try {
			...
			LOG.error("resolve file error",e);
			return null;
		}
	}
...
//		cellStyle.setFillForegroundColor(HSSFColor.SEA_GREEN.index);
	...
	public static void createExcelWithTemplate( Map<String,List<ImportErrorData>> dataList, String childDir,
													String template,String errorFileName,String prfix) {
		InputStream is = null;
		BufferedOutputStream bos = null;
		try {
			String filePath = System.getProperty("user.dir");
			is = new FileInputStream(filePath+File.separatorChar+"template" + File.separatorChar + template+"."+prfix);
			SXSSFWorkbook book = new SXSSFWorkbook(new XSSFWorkbook(is), 100);
			book.setCompressTempFiles(true);
...
//				CellStyle dateStyle = ExcelUtil.createDateStyle(book);
				if (!value.isEmpty()){
					for (int j = 0; j < value.size(); j++) {
						Row row = sheet.createRow(j + 1);
						String[] content  = value.get(j).getContent();
						int cnt=0;
						...
						CellStyle cellStyle = contentCellStyle;
						Cell cell = row.createCell(cnt);
						cell.setCellValue(value.get(j).getDataErrorInfo());
						cell.setCellStyle(cellStyle);
					}
				}
			});

			File outputFile = new File(filePath + File.separatorChar+"error" + File.separatorChar + childDir+File.separatorChar + errorFileName+"."+prfix);
		...
		if (cell != null) {
			...
			case STRING:
				try{
					...
					cellDatas[colNum] = cell.getStringCellValue();
				}
				break;
			default:
				break;
